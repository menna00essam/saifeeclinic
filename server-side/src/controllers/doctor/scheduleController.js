    ...
    for (const slot of available_slots) {
      ...
      if (!start_time || !end_time || !weekday) {
        return responseHandler.error(
          res,
          "Start time, end time, and weekday are required for each slot",
          400
        );
      }
      ...
      if (!validWeekdays.includes(weekday)) {
        return responseHandler.error(
          res,
          "Invalid weekday. Must be one of: " + validWeekdays.join(", "),
          400
        );
      }
      ...
      if (
        ...
        return responseHandler.error(
          res,
          "Time format for slots must be HH:MM",
          400
        );
      }
          ...
          if (appointment && appointment.patient_id) {
            await notificationService.sendToUser(
              ...
              "schedule_conflict",
              {
                ...
                appointmentTime: new Date(appointment.appointment_date)
                  .toTimeString()
                  .slice(0, 5),
                message:
                  "Your doctor has updated their schedule. Please contact the clinic to confirm your appointment.",
              },
              "email"
            );
          }
...
exports.getDoctorSchedule = async (req, res) => {
  try {
    ...
    return responseHandler.error(
      res,
      "Server Error while fetching schedule",
      500,
      err
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (upcomingAppointments.length > 0) {
        const conflictingAppointments = upcomingAppointments.map(
          (appointment) => ({
            patientName: `${appointment.patient_id.first_name} ${appointment.patient_id.last_name}`,
            date: new Date(appointment.appointment_date).toDateString(),
            time: new Date(appointment.appointment_date)
              .toTimeString()
              .slice(0, 5),
          })
        );
          ...
          await notificationService.sendToUser(
            ...
            "schedule_conflict",
            {
              ...
              appointmentTime: new Date(appointment.appointment_date)
                ...
                .slice(0, 5),
              message:
                "Your doctor has cleared their schedule. Please contact the clinic regarding your appointment.",
            },
            "email"
          );
        }
      ...
      console.error(
        "Error sending schedule clear notifications:",
        notificationError
      );
    }
    ...
    return responseHandler.error(
      res,
      "Server Error while clearing schedule",
      500,
      err
    );
  }
